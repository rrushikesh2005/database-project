...
const Navbar1 = ({ onLogout }) => {
  ...
  const [profilePicture, setProfilePicture] = useState(default_addr);
  ...
  useEffect(() => {
    const fetchProfilePicture = async () => {
      try {
            ...
            setProfilePicture(data.profile_picture || default_addr);
        } catch (error) {
            console.error("Error fetching profile picture:", error);
            setProfilePicture(default_addr);
        }
    };
  ...
  return (
    <div className="navbar1">
      <div className="navbar1-left">
        <span className="app-name">Movie Rating App</span>
      </div>
      <div className="navbar1-right">
        ...
        <button className="logout1-button" onClick={onLogout}>
          Logout
        </button>
      </div>
    </div>
