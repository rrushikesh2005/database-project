    ...
    const fetchUsers = async () => {
      ...
      try {
        const response = await fetch(
          `${apiUrl}/api/friends/search?query=${encodeURIComponent(searchQuery)}`,
          {
            method: "GET",
            credentials: "include",
  ...
  return (
    <div className="home-container">
      ...
      <div className="topbar">
        <h2 className="logo">Friends</h2>
      </div>

      <h1 className="heading">Find New Friends</h1>
      ...
      <div className="grid">
        ...
        {searchResults.map((user, index) => (
          <div key={index} className="card">
            <img
              src={user.profile_picture || default_addr}
              alt={user.username}
              className="image"
              onError={(e) => {
                e.target.src = default_addr;
              }}
            />
