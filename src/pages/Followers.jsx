...
const Followers = () => {
  ...
  useEffect(() => {
    const fetchFollowers = async () => {
      try {
        const response = await fetch(`${apiUrl}/api/user/followers`, {
          method: "GET",
  ...
  return (
    <div className="home-container">
      <div className="topbar">
        <h2 className="logo">Your Followers</h2>
      </div>
          ...
          <ul>
            {filteredFollowers.map((follower, index) => (
              <li
                ...
                onClick={() => navigate(`/profile1/${follower.username}`)}
              >
                <img
                  src={follower.profile_picture || default_addr}
